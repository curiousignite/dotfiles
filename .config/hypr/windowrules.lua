...
window_rule("idle-inhibit-media-players", {
	class = "^(.*celluloid.*)$|^(.*mpv.*)$|^(.*vlc.*)$",
}, {
	idle_inhibit = "fullscreen",
})

window_rule("idle-inhibit-spotify", {
	class = "^(.*[Ss]potify.*)$",
}, {
	idle_inhibit = "fullscreen",
})

window_rule("idle-inhibit-browsers", {
	class = "^(.*LibreWolf.*)$"
		.. "|^(.*floorp.*)$"
		.. "|^(.*brave-browser.*)$"
		.. "|^(.*firefox.*)$"
...
window_rule("default-window-opacity", {
	class = ".*",
}, {
	opacity = "1.00 override 1.00 override",
})

window_rule("ghostty-opacity", {
	class = ".*ghostty.*",
}, {
	opacity = "0.90 override 0.90 override",
})

window_rule("kitty-opacity", {
	class = ".*kitty.*",
}, {
	opacity = "0.90 override 0.90 override",
})
...
window_rule("dolphin-progress-dialog", {
...
window_rule("dolphin-copy-dialog", {
	class = "^(org.kde.dolphin)$",
...
float_title("firefox-about-dialog", "^(About Mozilla Firefox)$")

float_title("browser-devtools", "^(DevTools)$")

window_rule("firefox-library", {
	class = "^(firefox)$",
	title = "^(Library)$",
}, {
	float = true,
...
window_rule("picture-in-picture", {
...
window_rule("chromium-extension-popup", {
...
float_class("xdg-desktop-portal-gtk", "^(xdg-desktop-portal-gtk)$")

float_class("whatsie", "^(com.ktechpit.whatsie)$")

-- Float Steam auxiliary windows first.
float_class("steam-windows", "^(steam)$")

-- Main Steam window remains tiled.
-- Later rule overrides generic Steam floating rule.
window_rule("steam-main-window", {
	class = "^(steam)$",
	title = "^(Steam)$",
}, {
	tile = true,
...
blur_layer("blur-rofi", "^(rofi)$", 0)

blur_layer("blur-notifications", "^(notifications)$", 0)

blur_layer("blur-swaync-notification-window", "^(swaync-notification-window)$", 0)

blur_layer("blur-swaync-control-center", "^(swaync-control-center)$", 0)

blur_layer("blur-logout-dialog", "^(logout_dialog)$", nil)
--------------------------------------------------
-- Capture overlays
--------------------------------------------------
...
window_rule("mtga-tools", {
	class = "^(mtgatool-desktop)$",
}, {
	float = true,
})
